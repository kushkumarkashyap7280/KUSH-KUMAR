...
import { listPosts, deletePost, updatePost } from "../apis/posts";
...
export default function PostsManager() {
  ...
  const onTogglePublish = async (row) => {
    try {
      const res = await updatePost(row._id, { published: !row.published });
      const updated = res.data?.data?.post || row;
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {sorted.map((row, i) => (
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <div className="truncate text-sm font-semibold">
                      {row.title} · <span className="text-white/80">{row.platform}</span>
                    </div>
                    <a
                      href={row.link}
                      target="_blank"
                      rel="noreferrer"
                      className="text-xs text-blue-200 underline hover:text-blue-100"
                    >
                      Link
                    </a>
                  </div>
              ...
              {row.excerpt && (
                <div className="mt-3 line-clamp-2 text-sm text-white/70">{row.excerpt}</div>
              )}
